...
export class HomeComponent {
  ...
  getAllDoctor(){
    ...
    if(this.selectedDepartmentValue>0){
      this.httpService.post<DoctorModel[]>("Appointments/GetAllDoctorsByDepartment",{departmentValue:+this.selectedDepartmentValue},(res)=> {
  ...
  getAllAppointments(){
    if(this.selectedDoctorId){
        this.httpService.post<AppointmentModel[]>("Appointments/GetAllByDoctorId",{doctorId:this.selectedDoctorId},(res)=> {
  ...
  getPatient(){
    this.httpService.post<PatientModel>("Appointments/GetPatientByIdentityNumber",{identityNumber:this.createModel.identityNumber},(res)=> {
      if(res.data===null) {
  ...
  create(form:NgForm){
    if(form.valid){
      this.httpService.post<string>("Appointments/Create",this.createModel,res=> {
  ...
  onAppointmentDeleting(e:any){
    ...
    this.swal.callSwal("Delete Appointment?",`You want to delete ${e.appointmentData.patient.fullName} this appointment?`,()=>{
      this.httpService.post("Appointments/DeleteById",{id:e.appointmentData.id},res=> {
  ...
  onAppointmentUpdating(e:any){
    ...
    this.httpService.post<string>("Appointments/Update",data,res=>{
      this.swal.callToast(res.data);
      this.getAllAppointments();
    });
  }
